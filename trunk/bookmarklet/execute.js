...
function hangul_ime_execute(){
	...
	var i;
	for( i=0;i<textareas.length;i++ ) textareas[i].hangulObj = new hangulIME( textareas[i] );
	for( i=0;i<inputs.length;i++) if( inputs[i].type == "text" ) inputs[i].hangulObj = new hangulIME( inputs[i] );
...
function hangul_ime_toggle(){
	...
	var inputs = document.getElementsByTagName('input');
	var i;
	for( i=0;i<textareas.length;i++ ) textareas[i].hangulObj.toggleIME();
	for( i=0;i<inputs.length;i++) if( inputs[i].type == "text" ) inputs[i].hangulObj.toggleIME();
	var credit_obj = $("hangul_ime_credit");
